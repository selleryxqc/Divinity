...
namespace driver {
	...
	private:
		...
		bool send_control( DWORD timeout = 3000 ) {
			...
			if ( WaitForSingleObject( m_response_semaphore, timeout ) != WAIT_OBJECT_0 ) {
		...
		bool read_virtual_single( void* dst, std::uint64_t va, std::size_t size ) {
			...
			m_control_data->m_request_type = control::control_type::read_virtual;
	...
	public:
		...
		bool initialize( ) {
			...
			if ( !nt_dll ) {
				logging::print( oxorany( "Could not load ntdll" ) );
				return false;
			}

			auto nt_update_wnf_state_data = reinterpret_cast< nt_update_wnf_state_data_t >( 
				GetProcAddress( nt_dll, oxorany( "NtUpdateWnfStateData" ) ) 
				);
			if ( !nt_update_wnf_state_data ) {
				logging::print( oxorany( "Could not get NtUpdateWnfStateData" ) );
				return false;
			}

			WNF_STATE_NAME_INTERNAL state_name{ };
			state_name.Data[ 0 ] = static_cast< std::uint32_t >( 0x0d83063ea3be0875 & 0xFFFFFFFF );
			state_name.Data[ 1 ] = static_cast< std::uint32_t >( ( 0x0d83063ea3be0875 >> 32 ) & 0xFFFFFFFF );

			if ( nt_update_wnf_state_data(
				reinterpret_cast< WNF_STATE_NAME* >( &state_name ),
				&control_initialize,
		...
		std::uint32_t get_process_id( std::wstring target_process ) {
			...
			do {
				if ( !target_process.compare( process_entry.szExeFile ) )
		...
		std::uint32_t get_process_id( eprocess_t* eprocess ) {
			m_control_data->m_request_type = control::control_type::get_process_id;
			m_control_data->m_process = eprocess;

			if ( !send_control( ) )
				return -1;

			return m_control_data->m_process_id;
		}
		...
		std::uint64_t swap_directory_table_base( std::uint64_t new_directory_table_base ) {
			m_control_data->m_request_type = control::control_type::swap_directory_table_base;
			m_control_data->m_address = new_directory_table_base;

			if ( !send_control( ) )
				return 0;

			return m_control_data->m_address1;
		}

		std::uint64_t get_directory_table_base( eprocess_t* eprocess, bool swap = true ) {
			...
			if ( swap ) {
				swap_directory_table_base( m_control_data->m_address );
			}

			return m_control_data->m_address;
		}

		std::pair < eprocess_t*, std::uint64_t > get_hyperspace_context( eprocess_t* eprocess, bool swap = false ) {
			m_control_data->m_request_type = control::control_type::hyperspace_get_context;
			m_control_data->m_process = eprocess;

			if ( !send_control( 5000 ) )
				return { };

			const auto directory_table_base = m_control_data->m_address;
			if ( swap && directory_table_base ) {
				swap_directory_table_base( directory_table_base );
			}

			return { m_control_data->m_process, directory_table_base };
		}
		...
		bool attach_process( std::wstring target_process ) {
			...
			if ( IsIconic( this->m_process_window ) )
				ShowWindow( this->m_process_window, SW_RESTORE );

			SetForegroundWindow( this->m_process_window );
			BringWindowToTop( this->m_process_window );
			SetFocus( this->m_process_window );
		...
		std::uint32_t lookup_thread_by_process( eprocess_t* process ) {
			...
			if ( Thread32First( snapshot, &te32 ) ) {
				do {
					if ( te32.th32OwnerProcessID != this->m_process_id )
						continue;
			...
			if ( !target_thread_id ) {
				logging::print( oxorany( "No thread found" ) );
				return -1;
			}

			auto thread_object = this->lookup_thread( target_thread_id );
			if ( !thread_object ) {
				logging::print( oxorany( "Could not get thread object (%d)" ), target_thread_id );
				return -1;
			}

			logging::print( oxorany( "Found best thread TID: %d object: 0x%llx" ),
				target_thread_id, thread_object );
	...
	public:
		...
		ethread_t* lookup_thread( std::uint32_t thread_id ) {
			...
			m_control_data->m_thread_id = thread_id;
		...
		bool get_thread_context( ethread_t* thread, CONTEXT* out_context ) {
			...
			m_control_data->m_request_type = control::control_type::get_thread_context;
			m_control_data->m_thread = thread;
		...
		bool clone_in_hyperspace( eprocess_t* process ) {
			m_control_data->m_request_type = control::control_type::clone_in_hyperspace;
			m_control_data->m_process = process;
		...
		std::uint64_t allocate_virtual( eprocess_t* process, std::uint64_t size ) {
			m_control_data->m_request_type = control::control_type::hyperspace_allocate_virtual;
			m_control_data->m_process = process;
			m_control_data->m_size = size;
		...
		bool free_virtual( eprocess_t* process, std::uint64_t address ) {
			m_control_data->m_request_type = control::control_type::hyperspace_free_virtual;
			m_control_data->m_process = process;
			m_control_data->m_address = address;
		...
		bool remap_pages( eprocess_t* process, std::uint64_t address, std::uint64_t size ) {
			m_control_data->m_request_type = control::control_type::hyperspace_remap_pages;
			m_control_data->m_process = process;
			m_control_data->m_address = address;
			m_control_data->m_size = size;
		...
		std::uint64_t expose_pages( eprocess_t* process, eprocess_t* process2, std::uint64_t address, std::uint64_t size ) {
			m_control_data->m_request_type = control::control_type::hyperspace_expose_pages;
			m_control_data->m_process = process;
			m_control_data->m_process2 = process2;
			m_control_data->m_address = address;
			m_control_data->m_address1 = 0;
			m_control_data->m_size = size;

			if ( !send_control( 10000 ) )
				return false;

			return m_control_data->m_address1;
		}

		std::uint64_t expose_clone_pages( eprocess_t* process, eprocess_t* process2, std::uint64_t address, std::uint64_t size ) {
			m_control_data->m_request_type = control::control_type::hyperspace_clone_pages;
			m_control_data->m_process = process;
			m_control_data->m_process2 = process2;
			m_control_data->m_address = address;
			m_control_data->m_address1 = 0;
		...
		bool rexpose_pages( eprocess_t* process, eprocess_t* process2 ) {
			m_control_data->m_request_type = control::control_type::hyperspace_rexpose_pages;
			m_control_data->m_process = process;
			m_control_data->m_process2 = process2;
		...
		bool hide_pages( std::uint64_t base_address, std::uint64_t size ) {
			m_control_data->m_request_type = control::control_type::hide_process_pages;
		...
		std::uintptr_t get_process_module( const wchar_t* module_name ) {
			...
			do {
				...
				if ( entry.m_base_dll_name.m_length > 0 && entry.m_base_dll_name.m_length < MAX_PATH * 2 ) {
					...
					if ( read_memory( entry.m_base_dll_name.m_buffer, module_name_buffer, entry.m_base_dll_name.m_length ) ) {
			...
			do {
				...
				if ( entry.m_base_dll_name.m_length > 0 &&
					...
					if ( read_memory( entry.m_base_dll_name.m_buffer,
						...
						module_info_t info{};
						info.base = reinterpret_cast< std::uint64_t >( entry.m_dll_base );
